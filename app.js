...
function lectureRows(a){
  ...
  return a.map(i=>'<tr data-search="'+esc((i.title+" "+i.chapter+" "+i.day).toLowerCase())+'" data-status="'+(Number(i.progress||0)>=100?"done":Number(i.progress||0)>0?"doing":"todo")+'"><td><label class="donewrap"><input class="donecheck" type="checkbox" '+(Number(i.progress||0)><span>Done</span></label></td><td><b>'+esc(subject(i.subject)?.code||i.subject)+'</b></td><td>'+esc(i.no||"—")+'</td><td>'+esc(i.title)+'</td><td>'+duration(i.duration)+'</td><td><div class="progress" style="width:120px"><i style="width:'+Number(i.progress||0)+'%"></i></div><small>'+Number(i.progress||0)+'%</small></td><td><small>R1 '+Number(i.rev?.r1||0)+'% • R2 '+Number(i.rev?.r2||0)+'% • R3 '+Number(i.rev?.r3||0)+'%</small></td><td><button class="ghost" onclick="editItem(\''+i.id+'\')">Open</button></td></tr>').join("");
}
